...
class LSTMDaemon:
    def __init__(self):
        ...
        self.model, self.scaler_feature, self.scaler_regressor_target = None, None, None
        self._load_models()

    def _load_models(self):
        print("Loading combined model and scalers...")
        try:
            self.scaler_feature = joblib.load(SCALER_FILE_FEATURE)
            print(f"✓ Feature scaler loaded successfully.")
            
            self.model = CombinedLSTM(INPUT_FEATURES, HIDDEN_SIZE, NUM_LAYERS, NUM_CLASSES, OUTPUT_STEPS)
            checkpoint = torch.load(MODEL_FILE, map_location=self.device)
            self.model.load_state_dict(checkpoint['model_state'])
            self.model.to(self.device).eval()
            self.scaler_regressor_target = joblib.load(SCALER_FILE_REGRESSION_TARGET)
            print(f"✓ Combined LSTM model and target scaler loaded successfully.")
        except Exception as e:
            print(f"FATAL: Could not load models/scalers: {e}."); traceback.print_exc(); sys.exit(1)
    ...
    def _get_combined_prediction(self, features: list, current_price: float, atr: float) -> dict:
        ...
        with torch.no_grad():
            regression_output, classification_logits = self.model(tensor)

        unscaled_predictions = self.scaler_regressor_target.inverse_transform(regression_output.cpu().numpy())[0]
        ...
        probabilities = torch.softmax(classification_logits, dim=1)[0].cpu().numpy()
    ...
    def _handle_request(self, filepath: str):
        ...
        try:
            ...
            if not all([action, features, isinstance(current_price, float), isinstance(atr, float)]):
                raise ValueError("Request JSON missing/invalid keys.")
            ...
            if action == "predict_combined":
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ✓ Responded to {request_id} (Combined)")
            else:
    ...
    def run(self):
        print("\n--- Advanced Combined LSTM Daemon is running. ---")
        while True:
            try:
            ...
            except Exception: print("An error occurred in the main loop."); traceback.print_exc(); time.sleep(5)
